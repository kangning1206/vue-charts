...
export default {
  ...
  mounted() {
    ...
    // Remove duplicate by data name.
      const mapByDataName = {};
      const mapOnDim = { x: {}, y: {}, xy: {} };
      echarts.util.each(params, (item) => {
      ...
      echarts.util.each(mapByDataName, (mapItem, dataName) => {
      ...
      const resultHTML = [];
      echarts.util.each([['xy', 'CROSS'], ['x', 'V LINE'], ['y', 'H LINE']], (dimDefine) => {
        const html = [];
    ...
    const fieldNames = schema.map(item => item.name).slice(2);
    ...
    setTimeout(() => {
      data = normalizeData(db).slice(0, 1000);
